...
- **`MiniAssembler.java`**: The final stage. It maps the 3AC instructions to a custom **Virtual Machine ISA**. It handles register allocation (R1-R16) and generates a machine-readable binary/hex-like representation.

### 📂 `com.example.pipeline.service` (The Orchestrator)
- **`PipelineService.java`**: This is the "brain" of the application. It ensures a **Fail-Fast** architecture; if the Macro expansion fails, the pipeline stops immediately. It also constructs the **Execution Timeline**, capturing the state of the symbol table at every step to power the frontend XAI tooltips.
...
4.  **Phase 3 (Assembly)**: 3-Address codes are translated into machine instructions (e.g., `MOV R1, [X]`, `ADD R1, [Y]`).
5.  **Persistence**: The entire run (Input -> Result) is saved in `compiler.db` using JPA, allowing for historical analysis.

---

## 4. UI/UX Philosophy: The "Marimba" Design
The frontend uses a **Glassmorphic** approach (blurred backgrounds, translucent layers) to symbolize the "Transparency" of the compiler.
- **Explainable AI (XAI)**: Every line of the generated 3AC and Assembly code is interactive. Hovering over a line triggers a popup that explains *why* that code was generated and what it does in plain English.
- **Story View**: Converts the technical logs into a narrative "story" of the code's execution, making it accessible to non-experts.
